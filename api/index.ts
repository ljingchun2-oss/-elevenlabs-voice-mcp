...
const SPEAK_DESCRIPTION =
  ...
  "Optional: if a longer passage contains multiple clearly distinct emotional scenes, you may mark the breaks with a standalone --- line between them, so each scene can later be identified and regenerated on its own. This is not required — only do it when the scenes are genuinely different, not for every segment split.\n\n" +
